  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm dark:shadow-lg p-6 transition-colors">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex bg-zinc-100 dark:bg-zinc-800 p-1 rounded-lg border border-zinc-200 dark:border-zinc-700">
          <button 
            ...
            className={`p-1.5 rounded-md transition-colors ${viewMode === 'list' ? 'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-300'}`}
            title="List View"
          >
            <List size={18} />
          </button>
          <button 
            ...
            className={`p-1.5 rounded-md transition-colors ${viewMode === 'calendar' ? 'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-300'}`}
            title="Calendar View"
          >
            <Grid size={18} />
          </button>
      ...
      {viewMode === 'list' && (
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="flex-1 relative group">
            <Search className="absolute left-3.5 top-1/2 -tranzinc-y-1/2 text-zinc-400 group-focus-within:text-blue-500 transition-colors" size={18} />
            <input 
              type="text" 
              placeholder="Search transactions..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-12 py-2.5 bg-zinc-50 hover:bg-zinc-100 dark:bg-zinc-800/50 dark:hover:bg-zinc-800 border border-zinc-200 dark:border-zinc-700/50 rounded-xl text-zinc-900 dark:text-white placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all duration-200 shadow-sm"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery("")}
                className="absolute right-2.5 top-1/2 -tranzinc-y-1/2 p-1.5 text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200 bg-zinc-200/50 hover:bg-zinc-200 dark:bg-zinc-700/50 dark:hover:bg-zinc-700 rounded-full transition-all duration-200 flex items-center justify-center scale-95 hover:scale-100"
                title="Clear search"
              >
                <X size={14} />
              </button>
          ...
          <div className="flex bg-zinc-100/80 dark:bg-zinc-800/80 p-1.5 rounded-xl border border-zinc-200/80 dark:border-zinc-700/50 self-start sm:self-auto w-full sm:w-auto shadow-inner">
            <button 
              onClick={() => setTimeMode('month')}
              className={`flex-1 sm:flex-none px-5 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${timeMode === 'month' ? 'bg-white dark:bg-zinc-700 text-blue-600 dark:text-blue-400 shadow-sm ring-1 ring-zinc-200/50 dark:ring-zinc-600' : 'text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200 hover:bg-zinc-200/50 dark:hover:bg-zinc-700/50'}`}
            >
              This Month
            </button>
            <button 
              onClick={() => setTimeMode('all')}
              className={`flex-1 sm:flex-none px-5 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${timeMode === 'all' ? 'bg-white dark:bg-zinc-700 text-blue-600 dark:text-blue-400 shadow-sm ring-1 ring-zinc-200/50 dark:ring-zinc-600' : 'text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200 hover:bg-zinc-200/50 dark:hover:bg-zinc-700/50'}`}
            >
              All Time
            </button>
          </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 flex flex-col items-center justify-center border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-xl">
          <div className="bg-zinc-100 dark:bg-zinc-800 p-4 rounded-full mb-4">
            <Wallet size={32} className="text-zinc-400 dark:text-zinc-500" />
          </div>
        ...
        <div className="space-y-4">
          {filteredTransactions.slice((page - 1) * ITEMS_PER_PAGE, page * ITEMS_PER_PAGE).map(tx => {
            ...
            return (
              <div 
                ...
                className="flex items-center justify-between p-4 rounded-xl bg-zinc-50 dark:bg-zinc-800/50 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors border border-zinc-100 dark:border-zinc-700/50 cursor-pointer"
              >
                ...
                <div className="flex flex-col items-end flex-shrink-0">
                  <div className={`font-bold text-base sm:text-lg ${tx.type === TransactionType.DEBIT ? 'text-zinc-900 dark:text-white' : 'text-emerald-600 dark:text-emerald-400'}`}>
                    {tx.type === TransactionType.DEBIT ? '-' : '+'}₹{tx.amount.toFixed(2)}
                  </div>
                  {tx.paymentMode && (
                    <div className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5 px-2 py-0.5 rounded bg-zinc-200 dark:bg-zinc-700">
                      {tx.paymentMode}
                    </div>
                  )}
                </div>
          ...
          <div className="pt-4 mt-4 border-t border-zinc-100 dark:border-zinc-800 flex flex-col sm:flex-row items-center justify-between gap-4">
            <span className="text-sm text-zinc-500 dark:text-zinc-400 font-medium bg-zinc-100 dark:bg-zinc-800/50 px-3 py-1.5 rounded-lg">
            ...
            {filteredTransactions.length > ITEMS_PER_PAGE && (
              <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-4">
                <button 
                  ...
                  disabled={page === 1}
                  className="px-4 py-2 text-sm font-medium text-zinc-600 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800 rounded-lg disabled:opacity-50 transition-colors hover:bg-zinc-200 dark:hover:bg-zinc-700"
                >
                  Previous
                </button>
