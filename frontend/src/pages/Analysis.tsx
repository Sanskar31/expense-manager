  ...
  return (
    <div className="min-h-screen pb-20 sm:pb-8">
      ...
      <main className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 mt-4 sm:mt-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-zinc-900 p-6 rounded-3xl shadow-sm border border-zinc-200 dark:border-zinc-800">
          <div>
            <h1 className="text-3xl font-bold text-zinc-900 dark:text-white flex items-center gap-2">
              <TrendingUp className="text-blue-600 dark:text-blue-400" />
              Analysis Hub
            </h1>
            <p className="text-zinc-500 dark:text-zinc-400 mt-2">Discover your spending patterns and trends.</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            ...
            {(startDate || endDate) && (
              <button 
                onClick={handleReset}
                className="flex items-center gap-2 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 px-4 py-2 rounded-lg text-sm font-medium transition-colors"
              >
        ...
        {transactions.length === 0 ? (
          <div className="text-center py-16 flex flex-col items-center justify-center border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl bg-white/50 dark:bg-zinc-900/50">
            ...
            <h3 className="text-xl font-bold text-zinc-900 dark:text-white mb-2">Not Enough Data Yet</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {/* YoY Card */}
            {yoyData && (
              <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-800 shadow-sm col-span-1 lg:col-span-2 flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-zinc-500 mb-1">Year-over-Year ({yoyData.currentYear} vs {yoyData.lastYear})</p>
                  <div className="flex items-baseline gap-4 mt-2">
                    <h3 className="text-3xl font-bold text-zinc-900 dark:text-white">₹{yoyData.currYearSpent.toFixed(0)}</h3>
                    <div className="text-sm text-zinc-500">vs ₹{yoyData.lastYearSpent.toFixed(0)}</div>
                  </div>
                </div>
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-800 shadow-sm col-span-1 lg:col-span-2 flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-zinc-500 mb-1">Daily Average Spending</p>
                <h3 className="text-3xl font-bold text-zinc-900 dark:text-white mt-2">₹{dailyAverage.toFixed(2)}</h3>
              </div>
            ...
            {/* Top 5 Donut Chart */}
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl shadow-sm border border-zinc-200 dark:border-zinc-800 col-span-1 lg:col-span-2">
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-4">Top Categories</h3>
            ...
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl shadow-sm border border-zinc-200 dark:border-zinc-800 col-span-1 md:col-span-2 lg:col-span-4">
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-6">Total Spending & Investing Trend</h3>
              <div className="h-72 w-full">
                {monthChartData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={monthChartData}>
                      ...
                      <Legend 
                        ...
                        height={36} 
                        iconType="circle"
                        formatter={(value) => <span className="text-zinc-600 dark:text-zinc-300 capitalize">{value}</span>}
                      />
                  ...
                  <div className="h-full flex items-center justify-center text-zinc-400">Not enough data to show trends.</div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl shadow-sm border border-zinc-200 dark:border-zinc-800 col-span-1 md:col-span-2 lg:col-span-4">
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-6">Top Categories Evolution</h3>
              <div className="h-72 w-full">
                {lineData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                            ...
                            <ul className="flex flex-wrap gap-4 justify-center pb-4">
                              {payload.map((entry, index: number) => (
                                <li key={`item-${index}`} className="group relative flex items-center justify-center cursor-pointer gap-2">
                                  ...
                                  <div className="sm:hidden absolute bottom-full mb-2 left-1/2 -tranzinc-x-1/2 bg-zinc-900 dark:bg-zinc-700 text-white dark:text-zinc-100 text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50 shadow-lg border border-zinc-700/50 dark:border-zinc-600/50">
                                    {entry.value}
                                  </div>
                                </li>
                              ))}
