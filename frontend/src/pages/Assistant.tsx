    ...
    return (
      <div className="space-y-4 w-full">
        <p className="leading-relaxed whitespace-pre-wrap">
          {/* Simple markdown bold replacement for prototype */}
          {msg.text.split('**').map((part, i) => i % 2 === 1 ? <strong key={i} className="font-bold text-zinc-900 dark:text-white">{part}</strong> : part)}
        </p>
              ...
              {isModelMenuOpen && (
                <>
                  ...
                  <div className="absolute bottom-full left-0 mb-3 w-[140px] bg-white/95 dark:bg-zinc-900/95 backdrop-blur-2xl border border-zinc-200/60 dark:border-zinc-700/60 rounded-2xl shadow-xl shadow-black/5 dark:shadow-black/40 overflow-hidden flex flex-col p-1.5 z-20 animate-in fade-in slide-in-from-bottom-2">
                    <button
                      type="button"
                      onClick={() => { setModel('gemini-1.5-flash'); setIsModelMenuOpen(false); }}
                      className={`text-left px-3 py-2.5 text-[13px] font-semibold rounded-xl transition-all flex items-center gap-2 ${model === 'gemini-1.5-flash' ? 'bg-blue-50 dark:bg-blue-500/20 text-blue-700 dark:text-blue-300' : 'text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}
                    >
                      ⚡ Flash <span className="text-[10px] font-medium opacity-60">(Fast)</span>
                    </button>
                    <button
                      type="button"
                      onClick={() => { setModel('gemini-1.5-pro'); setIsModelMenuOpen(false); }}
                      className={`text-left px-3 py-2.5 text-[13px] font-semibold rounded-xl transition-all flex items-center gap-2 mt-0.5 ${model === 'gemini-1.5-pro' ? 'bg-purple-50 dark:bg-purple-500/20 text-purple-700 dark:text-purple-300' : 'text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}
                    >
